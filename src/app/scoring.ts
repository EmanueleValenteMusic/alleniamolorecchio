...
export function getScoreBounds(settings: SettingsState): ScoreBounds {
  ...
  const modeBonus = settings.playMode === 'quadriadi' ? 120 : (settings.playMode === 'triadi' || settings.playMode === 'tipo triade') ? 40 : 0;
  ...
  return {
    ...
    minScore: 16 + settings.slotCount * 7 + (settings.playMode === 'quadriadi' ? 28 : (settings.playMode === 'triadi' || settings.playMode === 'tipo triade') ? 12 : 0)
  };
}
...
export function computeScoreBreakdown(round: RoundState, settings: SettingsState): ScoreBreakdown {
  ...
  const modeMultiplier = settings.playMode === 'quadriadi' ? 1.3 : (settings.playMode === 'triadi' || settings.playMode === 'tipo triade') ? 1 : 0.72;
  ...
  const timePenalty = elapsedSeconds <= 10 ? 0 : Math.ceil((elapsedSeconds - 10) * (settings.playMode === 'quadriadi' ? 5 : (settings.playMode === 'triadi' || settings.playMode === 'tipo triade') ? 4 : 3));
...
export function getMistakePenalty(round: RoundState): number {
  ...
  return 6 + round.slotCount * 4 + (round.playMode === 'quadriadi' ? 12 : (round.playMode === 'triadi' || round.playMode === 'tipo triade') ? 4 : 0);
}
